    ...
    public List<Integer> findSubstring(String s, String[] words) {
        ...
        for (int i = 0; i <= s.length() - words.length * wordLength; i++) {
            ...
            for (int j = 0; j < words.length; j++) {
                ...
                if (seen.get(currentWord) > wordCounts.get(currentWord)) {
                    while (seen.get(currentWord) > wordCounts.get(currentWord)) {
                        String leftWord = s.substring(i + (j - wordCount + 1) * wordLength, i + (j - wordCount + 2) * wordLength);
