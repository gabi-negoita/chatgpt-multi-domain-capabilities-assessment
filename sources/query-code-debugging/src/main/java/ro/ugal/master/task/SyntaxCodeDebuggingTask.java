...
@Slf4j
@Component
public class SyntaxCodeDebuggingTask {

    private static final String DATASET = "syntax-code-debugging-dataset.json";
    ...
    public void start() throws URISyntaxException, IOException, InterruptedException {
        ...
        JsonNode node = mapper.readTree(new File(Objects.requireNonNull(SyntaxCodeDebuggingTask.class.getClassLoader().getResource(DATASET)).toURI()));
        ...
        for (var item : node) {
            ...
            var request = UserQueryRequest.builder()
                    ...
                    .input(content)
                    .inputType(UserQueryInputType.SYNTAX_DEBUGGING_PROBLEMS)
                    .result("Bug type: " + bugType + ". " + result)
                    .reference(UserQueryReference.NO_REFERENCE)
