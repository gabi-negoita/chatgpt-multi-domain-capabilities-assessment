...
@Slf4j
@Component
public class ArithmeticMultiplicationTask {

    private static final String DATASET = "arithmetic__mul.txt";
    ...
    public void start() throws URISyntaxException, FileNotFoundException, InterruptedException, JsonProcessingException {
        var scanner = new Scanner(new File(Objects.requireNonNull(ArithmeticMultiplicationTask.class.getClassLoader().getResource(DATASET)).toURI()));
        ...
        while (scanner.hasNextLine()) {
            ...
            var request = UserQueryRequest.builder()
                    .title("Mathematics: Multiplication")
                    .input(sanitizeString(query))
                    .inputType(UserQueryInputType.MATH_ARITHMETIC_MULTIPLICATION)
