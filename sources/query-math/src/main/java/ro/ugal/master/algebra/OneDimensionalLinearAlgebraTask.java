...
@Slf4j
@Component
public class OneDimensionalLinearAlgebraTask {

    private static final String DATASET = "algebra__linear_1d.txt";
    ...
    public void start() throws URISyntaxException, FileNotFoundException, InterruptedException, JsonProcessingException {
        var scanner = new Scanner(new File(Objects.requireNonNull(OneDimensionalLinearAlgebraTask.class.getClassLoader().getResource(DATASET)).toURI()));
        ...
        while (scanner.hasNextLine()) {
            ...
            var request = UserQueryRequest.builder()
                    .title("Mathematics: One dimensional linear algebra")
                    .input(sanitizeString(query))
                    .inputType(UserQueryInputType.MATH_ALGEBRA_1D)
