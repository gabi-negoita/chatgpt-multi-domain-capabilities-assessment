package ro.ugal.master.comparison;
...
@Slf4j
@Component
public class ComparisonSortTask {

    private static final String DATASET = "comparison__sort.txt";
    ...
    public void start() throws URISyntaxException, FileNotFoundException, InterruptedException, JsonProcessingException {
        var scanner = new Scanner(new File(Objects.requireNonNull(ComparisonSortTask.class.getClassLoader().getResource(DATASET)).toURI()));
        ...
        while (scanner.hasNextLine()) {
            ...
            var request = UserQueryRequest.builder()
                    .title("Mathematics: Sorting")
                    .input(sanitizeString(query))
                    .inputType(UserQueryInputType.MATH_COMPARISON_SORT)
