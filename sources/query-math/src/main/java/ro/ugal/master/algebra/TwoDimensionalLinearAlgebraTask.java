package ro.ugal.master.algebra;
...
@Slf4j
@Component
public class TwoDimensionalLinearAlgebraTask {

    private static final String DATASET = "algebra__linear_2d.txt";
    ...
    public void start() throws URISyntaxException, FileNotFoundException, InterruptedException, JsonProcessingException {
        var scanner = new Scanner(new File(Objects.requireNonNull(TwoDimensionalLinearAlgebraTask.class.getClassLoader().getResource(DATASET)).toURI()));
        ...
        while (scanner.hasNextLine()) {
            ...
            var request = UserQueryRequest.builder()
                    .title("Mathematics: Two dimensional linear algebra")
                    .input(sanitizeString(query))
                    .inputType(UserQueryInputType.MATH_ALGEBRA_2D)
                    .result(sanitizeString(result))
                    .reference(UserQueryReference.MATHEMATICS_DATASET)
                    .confidence(UserQueryConfidence.LOW)
